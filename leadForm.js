document.addEventListener("DOMContentLoaded", function() {
    ...
    leadForm.addEventListener('submit', async function(event) {
        ...
        const selectedQuestions = Array.from(document.querySelectorAll('.optionButton[data-selected="true"]')).map(button => button.textContent).join(', ');
        const firstName = formData.get('first_name');
        const lastName = formData.get('last_name');
        const phone = formData.get('phone');
        const email = formData.get('email');
        const accountId = document.getElementById('dealership-logo-dialog').dataset.accountId;
